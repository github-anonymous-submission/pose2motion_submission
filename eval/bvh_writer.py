...
def write_bvh(parent, offset, rotation, position, names, frametime, order, path, endsite=None):
    ...
    def write_static(idx, prefix):
        ...
        if not has_child and '_end' not in names[idx]:
...
class WriterWrapper:
    ...
    def write(self, filename, rot, pos, offset=None, names=None, repr='quat'):
        ...
        :return:
        """
        if repr not in ['euler', 'quat', 'quaternion', 'matrix']:
        ...
        if repr == 'matrix':
            ...
            rot = R.from_matrix(rot).as_euler('ZXY', degrees=True).reshape(bs, -1, 3)
        ...
        if repr == 'repr6d' or repr == 'quat' or repr == 'quaternion':
            ...
            euler = R.from_quat(rot).as_euler('ZXY', degrees=True)
            rot = euler

        if names is None:
            names = ['%02d' % i for i in range(n_bone)]
        write_bvh(self.parents, offset, rot, pos, names, 1, 'ZXY', filename)